...
#define _SEMEP_HPP_
	...
	namespace SEMEP {
		...
		private:
			...
			enum MemberKind {Number, Constant, Variable, UnaryOperator,
                             BinaryOperator, LeftBracket, RightBracket};
			...
			public:
				Member(MemberKind k, MemberGlobal g, unsigned char p) :
                    global(g), kind(k), priority(p) {}
				virtual ~Member() {}
			...
			protected:
				double value;
			public:
				MemberOperand(MemberKind k, double d) :
                    Member(k, Operand, 0), value(d) {}
			...
			public:
				MemberOperator(MemberKind k, unsigned char p) :
                    Member(k, Operator, p) {}
			...
			public:
				MemberUnary(double (*f)(double x)) :
                    MemberOperator(UnaryOperator, 254), function(f) {}
				~MemberUnary() {}
			...
			public:
				MemberBinary(double (*f)(double x, double y), unsigned char p)
                    : MemberOperator(BinaryOperator, p), function(f) {}
				~MemberBinary() {}
				void evaluate(std::stack<double> &s) const {
					double x, y;
					y = s.top();
			...
			enum ErrorKind {UNKNOWN, EMPTY, OUTRANGE, UNKNOWNMEMBER,
                            OPERANDSTOGETHER, OPERATORSTOGETHER, BINARYFIRST,
                            OPERATORLAST, BRACKETMISMATCH, BRACKETSTOGETHER,
                            OPERANDLEFT, LEFTLAST, RIGHTFIRST, RIGHTOPERAND,
                            OPERATORRIGHT, RIGHTOPERATOR};
		...
		private:
			static void addBinary(std::string s,
...
extern int SEMEP_create(SEMEP_Expression *e, const char *c);
extern void SEMEP_free(SEMEP_Expression *e);

extern double SEMEP_eval(SEMEP_Expression e, ...);
extern double SEMEP_feval(SEMEP_Expression e, double r, double s, double t,
                          double u, double v, double w,
                          double x, double y, double z); // Fortran95

extern void SEMEP_build(SEMEP_Expression *e, const char *c);
extern void SEMEP_clear(SEMEP_Expression *e);
extern int SEMEP_isBuilt(SEMEP_Expression e);

extern void SEMEP_getLiteral(SEMEP_Expression e, char *c);
extern void SEMEP_getVarName(SEMEP_Expression e, char *c, unsigned int i);
extern void SEMEP_fgetVarName(SEMEP_Expression e, char *c, int i);  // Fortran95
extern unsigned int SEMEP_getVarCount(SEMEP_Expression e);
extern int SEMEP_fgetVarCount(SEMEP_Expression e); // Fortran95

extern int SEMEP_checkError(SEMEP_Expression e);
extern void SEMEP_whatError(SEMEP_Expression e, char *c);

extern void SEMEP_addConstant(const char *c, double x);
extern void SEMEP_addUnaryOperator(const char *c, double (*f)(double x));
